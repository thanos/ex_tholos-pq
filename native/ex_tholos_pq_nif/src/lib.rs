...
#[rustler::nif]
fn gen_recipient_keypair<'a>(env: Env<'a>, kid: String) -> NifResult<(rustler::Atom, (String, Binary<'a>))> {
...
#[rustler::nif]
fn gen_sender_keypair<'a>(env: Env<'a>, sid: String) -> NifResult<(rustler::Atom, (String, Binary<'a>))> {
...
fn encrypt<'a>(
    ...
    let recipients = recipients
        .map_err(|e| Error::Term(Box::new(format!("Failed to deserialize recipients: {:?}", e))))?;
...
fn decrypt<'a>(
    ...
    for pub_key_bytes in &allowed_sender_pub_keys {
        let sender_pub: tholos_pq::SenderPub = serde_cbor::from_slice(pub_key_bytes.as_slice())
            .map_err(|e| Error::Term(Box::new(format!("Failed to deserialize sender pub key: {:?}", e))))?;
    ...
    let mut plain_bin = OwnedBinary::new(plaintext.len()).ok_or(Error::Atom("allocation_failed"))?;
